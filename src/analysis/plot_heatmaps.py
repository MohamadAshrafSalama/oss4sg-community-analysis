...
def read_clusters(set_name: str):
    p = OUTDIR / "reports" / f"clusters_{set_name}.csv"
...
def plot_heatmap(set_name: str, absval: bool = False):
    ...
    ax.set_title(f"Spearman {'|rho|' if absval else 'rho'} – {set_name}")
    ...
    # Draw cluster boundaries using cluster id transitions
    cid_map = {}
    p = OUTDIR / "reports" / f"clusters_{set_name}.csv"
    if p.exists():
        with p.open(encoding='utf-8') as f:
    ...
    OUT = OUTDIR / "plots" / f"heatmap_{'abs_' if absval else ''}{set_name}.png"
