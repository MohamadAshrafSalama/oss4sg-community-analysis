...
def plot_dendrogram(set_name: str, cut: float):
    ...
    ax.axhline(cut, color='black', linestyle='--', linewidth=1, label=f'Cut at {cut:.2f}')

    ax.set_xticks(range(len(leaf_nodes)))
    ax.set_xticklabels([lf.label for lf in leaf_nodes], rotation=90, fontsize=8)
    ax.set_ylabel('distance (1 - |rho|)', fontsize=12)
    ax.set_title(f'Average-linkage dendrogram – {set_name} (RQ1+RQ2)', fontsize=14, fontweight='bold')
    ax.legend()
    ...
    if abs(cut - 0.30) < 0.01:  # 0.7 threshold
        OUT = OUTDIR/'plots'/f'dendrogram_tree_{set_name}_rq1_rq2_085.png'
    elif abs(cut - 0.15) < 0.01:  # 0.85 threshold  
        OUT = OUTDIR/'plots'/f'dendrogram_tree_{set_name}_rq1_rq2_085.png'
    else:
        OUT = OUTDIR/'plots'/f'dendrogram_tree_{set_name}_rq1_rq2.png'
