...
# Experiment 2.0 layout
BARE_REPOS_DIR = os.path.join(SCRIPT_DIR, "bare_repos", "oss4sg")
RESULT_DIR = os.path.join(SCRIPT_DIR, "prism", "oss4sg")
...
try:
    ...
    commits = []
    for line in run_res.stdout.strip().split('\n'):
        if line:
            parts = line.split('|', 4)
            if len(parts) == 5:
                commits.append({
